...
<p>When a user interacts with a <code>ViewSet</code> component, some preferences are kept in localstorage. This means user preferences are remembered per browser without having to use any kind of database or syncing service. By default, we store the transposition of each tune in a set, and the zoom level for the set at each device orientation.</p>
